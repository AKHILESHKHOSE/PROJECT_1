...
--1
--CREATING 'matches' TABLE IN A NON-PUBLIC SCHEMA 'myschema'  
--WITH REQUIRED FIELDS MENTIONED IN PROJECT_STATEMENT.txt
...
--2
--CREATING 'deliveries' TABLE IN A NON-PUBLIC SCHEMA 'myschema'  WITH REQUIRED FIELDS MENTIONED 
--IN PROJECT_STATEMENT.txt
...
--IMPORT DATA FROM CSV FILES AND VERIFYING THEM
--3
...
SELECT * FROM myschema.matches WHERE result = 'runs' AND result_margin > 100;
...
--11
--ADD NEW COLUMN 'ball result' TO deliveries_v02;
...
is_wicket, dismissal_kind, player_dismissed, fielder, extras_type,
 batting_team, bowling_team, ball_result, venue, date  FROM myschema.deliveries_v03 ;
...
SELECT CONCAT(id,' - ',inning,' - ',over,' - ',ball) AS ball_id,
 batsman, non_striker, bowler, batsman_runs, extra_runs, total_runs,
is_wicket, dismissal_kind, player_dismissed, fielder, extras_type,
 batting_team, bowling_team, ball_result, venue,
date  FROM myschema.deliveries_v03 ;
